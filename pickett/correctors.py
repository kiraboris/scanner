# python 2,3
#
# Some correctors for the Formatter class of catalog module
#
...
class Corrector(ABC):
    @abstractmethod
    def find_mergable_with_state(self, states, state): pass

    @abstractmethod
    def find_mergable_with_line(self, lines, line): pass

    @abstractmethod
    def merge_lines(self, blends): pass

    @abstractmethod
    def merge_states(self, blends): pass

    @abstractmethod
    def split_line(self, lines, line, flag_require_all_spilts): pass

    @abstractmethod
    def split_state(self, states, state): pass


class SpinSymmCorrector(Corrector):
    ...
    def split_line(self, dict_entries, entry, flag_require_all_spilts):
        """docstring"""
        ...
        if (self.__spin_symm(entry) == "A"):
            ...
            if (ql['K'] == 0 or qu['K'] == 0):
                ...
                if (np.sign(qu['K']) == -np.sign(ql['K'])):  # Ku == -Kl mod 2, correct
                    if (not flag_require_all_spilts):
                        result.append(entry)
                    else:
    ...
    def __spin_symm(self, entry):
        ...
        if hasattr(entry, "q_upper") and hasattr(entry, "q_lower"):
