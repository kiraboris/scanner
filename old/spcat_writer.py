...
def var_writermmw(wat,A,B,C,DJ,DJK,DK,dJ,dK,PhiJ,PhiJK,PhiKJ,PhiK,phiJ,phiJK,phiK,LJ,LJJK,LJK,LKKJ,LK,l1,l2,l3,l4,chiaa,chibbcc,spin):# Generates SPCAT input file with sextics, one quadrupole
    input_file = ""
    input_file += "anisole                                         Wed Mar Thu Jun 03 17:45:45 2010\n"
    input_file += "   26  430   51    0    0.0000E+000    1.0000E+005    1.0000E+000 1.0000000000\n"
    if wat == "s":
        input_file +="s   %s  1  0  99  0  1  1  1  1  -1   0\n" %(spin)    
    else:
        input_file +="a   %s  1  0  99  0  1  1  1  1  -1   0\n" %(spin)
    ...
    if wat == "s":
        input_file += "           50000  %s""E-003 1.0E-010 /-dK   \n" %(dK)
    else:
...
choices = ["Prolate asymmetric, no hyperfine", "Prolate asymmetric, one quadrupolar nucleus", "Prolate asymmetric, with sextics and octics, one quadrupolar nucleus (for mmW)", "Prolate asymmetric, with sextics and octics, one quadrupolar nucleus, Watson S-reduction"]    
...
if choice == choices[2]:
    ...
    varValues = multenterbox(varMsg, title, varNames)
    int_writer(intValues[0],intValues[1],intValues[2],"00",intValues[5],"-10",QrotA(varValues[0],varValues[1],varValues[2],intValues[4]),intValues[3],intValues[4])
    var_writermmw("a",varValues[0],varValues[1],varValues[2],varValues[3],varValues[4],varValues[5],varValues[6],varValues[7],varValues[8],varValues[9],varValues[10],varValues[11],varValues[12],varValues[13],varValues[14],varValues[15],varValues[16],varValues[17],varValues[18],varValues[19],varValues[20],varValues[21],varValues[22],varValues[23],varValues[24],varValues[25],str(int(spinf))) 
if choice == choices[3]:
    spinMsg = "Enter the spin of the nucleus. Use decimal notation for half-integer spins (e.g. 1.5 for 3/2)."
    spin = enterbox(spinMsg)
    if float(spin) == math.floor(float(spin)):
        ...
        print('Input spin parameter is:'" "+str(spinf))   
    intMsg = "Enter all values below. Set all unused values to 0. Hint: Set Jmax arbitrarily high for a general prediction"
    varMsg = "Enter all values below in MHz unless otherwise specified. Set all unused values to 0. Uses Watson S-reduction."
    intNames = ["uA","uB","uC", "max freq (in GHz)", "temp (K)", "Jmax"]
    intValues = []
    intValues = multenterbox(intMsg, title, intNames)
    varNames = ["A (MHz)","B (MHz)","C (MHz)","-DJ (kHz)","-DJK (kHz)","-DK (kHz)","-dJ (kHz)","-dK (kHz)", "PhiJ (Hz)", "PhiJK (Hz)", "PhiKJ (Hz)", "PhiK (Hz)","phiJ (Hz)","phiJK (Hz)","phiK (Hz)","LJ (mHz)","LJJK (mHz)", "LJK (mHz)","LKKJ (mHz)","LK (mHz)","l1 (mHz)","l2 (mHz)", "l3 (mHz)","l4 (mHz)","1.5Chi_aa (MHz)", "0.25(Chi_bb-Chi_cc) (MHz)"]
    ...
    varValues = multenterbox(varMsg, title, varNames)
    int_writer(intValues[0],intValues[1],intValues[2],"00",intValues[5],"-10",QrotA(varValues[0],varValues[1],varValues[2],intValues[4]),intValues[3],intValues[4])
    var_writermmw("s",varValues[0],varValues[1],varValues[2],varValues[3],varValues[4],varValues[5],varValues[6],varValues[7],varValues[8],varValues[9],varValues[10],varValues[11],varValues[12],varValues[13],varValues[14],varValues[15],varValues[16],varValues[17],varValues[18],varValues[19],varValues[20],varValues[21],varValues[22],varValues[23],varValues[24],varValues[25],str(int(spinf))) 
